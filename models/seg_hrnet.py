...
def hrnet_w48(pretrained=False):
    import yaml
    hrnet_cfg = os.path.join('./models', 'model_info', 'hrnet_w48.yml')
...
def hrnet_w64(pretrained=False):
    import yaml
    hrnet_cfg = os.path.join('./models', 'model_info', 'hrnet_w64.yml')
    ...
    if pretrained:
        ...
        if os.path.exists(pretrained_weights):
            ...
            raise AssertionError(
                'Error: No pretrained weights found for HRNet64. \n Download weights from https://github.com/HRNet/HRNet-Image-Classification and save them to {}'.format(
                    pretrained_weights))
